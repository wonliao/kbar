...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.loadFooterView = [[[LoadingMoreFooterView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44.f)] autorelease];
...
{    
    ...
    for (int i = 0; i< numberOfColumns; i++)
    {
        ...
        //NSLog(@"currentPage(%d) rows(%d)", currentPage, rows);
...
- (void)reloadData2
{
    ...
    //self.isRefreshing = YES;
    //self.loadFooterView.showActivityIndicator = NO;
    //[self.refreshHeaderView egoRefreshScrollViewDidEndDragging:self];
    [self initialize];
    //self.isRefreshing = NO;
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
        ...
        while ( cell && ((cell.frame.origin.y - self.contentOffset.y) > 0.0001)) 
        {
            ...
            //NSLog(@"test 2(%d)(%d)", rowToDisplay, i);
        ...
        while (cell &&  ((cell.frame.origin.y + cell.frame.size.height - self.frame.size.height - self.contentOffset.y) <  0.0001)) 
		{
            ...
            //NSLog(@"test 3(%d)(%d)", rowToDisplay, i);
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    if( end.y > start.y ) {
        ...
        //[_flowdelegate showFloatUI:NO];
    } else {
        
        //[_flowdelegate showFloatUI:YES];
    }
