...
- (IBAction)doneAction:(id)sender
{
    ...
    UIImage *a = self.sourceImage;
    NSLog( @"a.description(%@)", a.description );
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIImage *transform =  [UIImage imageWithCGImage:resultRef];

            // 取得圖片 file size
            NSData *imgData = UIImagePNGRepresentation(transform);
            int imageSize = imgData.length;
            NSLog(@"SIZE OF IMAGE: %i ", imageSize);

            // 如果 file size 大於 2k，就進行壓縮
            if( imageSize > 2000 ) {

                float rate = 2000.0f / (float)imageSize;
                NSLog(@"rate: %f", rate);
                imgData = UIImageJPEGRepresentation(transform, rate);
            }

            int imageSize2 = imgData.length;
            NSLog(@"SIZE OF IMAGE2: %i ", imageSize2);
            ...
            NSString *userId = self.fbUID;
            NSString *pid = @"1";
            NSString *m_songTitle = @"test";
   
            NSString *urlString = [NSString stringWithFormat:@"http://54.200.150.53/kbar/import/upload_img.php?uid=%@&pid=%@&title=%@", userId, pid, m_songTitle];
            ...
            [request addData:imgData forKey:[NSString stringWithFormat:@"userfile"]];
            ...
            NSString *fileName = [NSString stringWithFormat:@"FB_%@.jpg", userId];
            NSString* imgFile = [documentsPath stringByAppendingPathComponent:fileName];
            
            [[NSFileManager defaultManager] removeItemAtPath:imgFile error:NULL];
            
            ImgPool* imgPool = [ImgPool sharedInstance];
            [imgPool.m_imgPool removeObjectForKey:userId];
