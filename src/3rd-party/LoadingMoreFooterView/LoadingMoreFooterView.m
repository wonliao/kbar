...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //self.textLabel.text = NSLocalizedString(@"上拉載入更多", @"Legen Sie mehr");
        self.textLabel.text = NSLocalizedStringFromTable(@"上拉載入更多", @"common", nil);
...
- (void) setShowActivityIndicator:(BOOL)showActivityIndicator {
    ...
    if (showActivityIndicator && !self.activityView) {
        ...
        //self.textLabel.text = NSLocalizedString(@"載入中...", @"Laden");
        self.textLabel.text = NSLocalizedStringFromTable(@"載入中...", @"common", nil);
    }
