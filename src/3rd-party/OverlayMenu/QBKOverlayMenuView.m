...
- (void)unfoldWithAnimationDuration:(float)duration
{
    [UIView animateWithDuration:duration animations:^{
        ...
        [_mainButton setBackgroundImage:[UIImage imageNamed:@"magic_white.png"] forState:UIControlStateNormal];
...
- (void)addButtonWithText:(NSString *)showText index:(NSInteger)index
{
    if (!_additionalButtons) _additionalButtons = [[NSMutableArray alloc] init];
    ...
    // Configuramos el botón
    //UIButton *newButton = [[UIButton alloc] initWithFrame:CGRectMake(posXBoton, posYBotonCentrado, QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_WIDTH, QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_HEIGHT)];
    UIButton *newButton = [[UIButton alloc] initWithFrame:CGRectMake(posXBoton, posYBotonCentrado, 60, 44)];
    
    //[newButton setBackgroundImage:[UIImage imageNamed:@"rw-button.png"] forState:UIControlStateNormal];
    [newButton setBackgroundColor: [UIColor yellowColor]];

    [newButton setTitle:showText forState:UIControlStateNormal];
